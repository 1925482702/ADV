...
def visualize_shift(img_6ch, bboxes, shift_gt, shift_mask, save_path):
    """可视化 shift 效果
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if shift_mask[i] > 0.5:
            ...
            # 计算平移后的边界框
            new_x1 = np.clip(x1 + dx, 0, 1)
            new_y1 = np.clip(y1 + dy, 0, 1)
            new_x2 = np.clip(x2 + dx, 0, 1)
            new_y2 = np.clip(y2 + dy, 0, 1)
    ...
    # 添加文字说明
    font = cv2.FONT_HERSHEY_SIMPLEX
    cv2.putText(result, "RGB", (10, 30), font, 1, (255, 255, 255), 2)
    cv2.putText(result, "IR", (w + 10, 30), font, 1, (255, 255, 255), 2)
...
def main():
    ...
    for i, rgb_path in enumerate(selected_images):
        ...
        visualize_shift(shifted_img, bboxes, shift_gt, shift_mask, output_path)
