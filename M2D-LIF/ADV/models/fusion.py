...
核心设计理念：
1. 基础融合：RGB + IR 特征相加或拼接
2. 残差学习：残差块学习"误差修正"，零初始化确保训练稳定
...
class MultiScaleFusion(nn.Module):
    ...
    def __init__(
        ...
        zero_init_residual: bool = True,  # 🚨 新增：残差块零初始化开关
    ):
        """
        参数:
            ...
            zero_init_residual: 是否对残差块进行零初始化（默认True）
        """
        super().__init__()
        ...
        # 残差学习块（每个尺度对应一个）
        # 🚨 核心改进：传入 zero_init 参数
        if use_residual:
            print("创建残差块（零初始化）:")
            self.residual_blocks = nn.ModuleList([
                ResidualFusionBlock(out_ch, zero_init=zero_init_residual)
