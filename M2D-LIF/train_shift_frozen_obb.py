...
# -*- coding: utf-8 -*-
"""
冻结 Backbone 版的 Shift OBB 训练脚本

用法：
    python train_shift_frozen_obb.py \
...
class FrozenShiftOBBTrainer(OBBTrainer):
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        """返回 ShiftOBBModel 并加载冻结的 backbone 权重"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train YOLOv8 OBB Shift with Frozen Backbone')
    ...
    parser.add_argument('--model', type=str, default='./model_yaml_obb/yolov8_obb_shift.yaml', help='模型配置文件')
...
def main():
    ...
    LOGGER.info("=" * 60)
    LOGGER.info("冻结 Backbone Shift OBB 训练")
    LOGGER.info("=" * 60)
    ...
    try:
        trainer = FrozenShiftOBBTrainer(overrides=train_args, rgb_teacher=args.rgb_teacher, ir_teacher=args.ir_teacher)
