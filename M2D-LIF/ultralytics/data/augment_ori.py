...
class RandomPerspective:
    ...
    def affine_transform(self, img, img_lwir, border):
        ...
        Modified for dual-modality (RGB + LWIR) synchronized augmentation.

        Args:
            img (ndarray): Input RGB image.
            img_lwir (ndarray): Input IR/LWIR image.
            border (tuple): Border dimensions.

        Returns:
            img (ndarray): Transformed RGB image.
            img_lwir (ndarray): Transformed IR/LWIR image.
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            ...
            # 🎯 强制内存连续化，解决 OpenCV _step >= minstep 报错
