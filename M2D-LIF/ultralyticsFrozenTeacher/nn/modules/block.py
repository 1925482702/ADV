...
class FrozenBackbone(nn.Module):
    """
    Frozen backbone wrapper that loads weights from a pretrained teacher model.
    The backbone layers (0-9 in standard YOLOv8) are frozen during training.
    
    Args:
        weight_path: Path to the pretrained teacher model (.pt file)
        c1: Input channels (default: 3 for single modality)
    """
    ...
    def __init__(self, weight_path=None, c1=3):
        ...
        # Standard YOLOv8 backbone structure (layers 0-9)
    ...
    def _load_pretrained_weights(self, weight_path):
        """Load weights from pretrained teacher model."""
        import torch
        ...
        # Map teacher backbone weights to our modules
        # Teacher layers 0-9 correspond to our backbone
        new_state = {}
        for name, param in self.named_parameters():
            # Map our parameter names to teacher's
        ...
        if new_state:
            ...
            print(f"Loaded {len(new_state)} parameters from {weight_path}")
        else:
            print(f"Warning: No matching parameters found in {weight_path}")
    ...
    def forward(self, x):
        ...
        p4 = self.stage4(p3)
        p5 = self.stage5(p4)
        return p2, p3, p4, p5  # Return features at multiple scales


class DualBranchFrozen(nn.Module):
    """
    Dual-branch frozen backbone for RGB-IR fusion.
    Uses two separate frozen backbones (RGB teacher and IR teacher).
    
    Args:
        rgb_weight: Path to RGB teacher weights
        ir_weight: Path to IR teacher weights
    """
    
    def __init__(self, rgb_weight=None, ir_weight=None):
        super().__init__()
        
        # Two frozen backbones for RGB and IR
        self.rgb_backbone = FrozenBackbone(weight_path=rgb_weight, c1=3)
        self.ir_backbone = FrozenBackbone(weight_path=ir_weight, c1=3)
        
        print(f"DualBranchFrozen initialized:")
        print(f"  RGB backbone: {rgb_weight}")
        print(f"  IR backbone: {ir_weight}")
    
    def forward(self, x):
        """
        Forward pass for dual-branch backbone.
        
        Args:
            x: Input tensor with shape [B, 6, H, W] (RGB + IR concatenated)
        
        Returns:
            Tuple of multi-scale features from both branches
        """
        # Split input into RGB and IR
        rgb_input = x[:, :3, :, :]  # First 3 channels
        ir_input = x[:, 3:, :, :]   # Last 3 channels
