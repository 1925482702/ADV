...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        ##### LIF #####
        # 检查模型中是否有 LIF 类型的模块，而不是检查路径字符串
        from ultralytics.nn.modules.block import LIF
        self.FIA = any(isinstance(m, LIF) for m in self.model.modules()) if isinstance(self.model, nn.Module) else False
        if self.FIA:
